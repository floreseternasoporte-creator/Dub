"""
Pipeline de Doblaje — Railway Edition
El modelo SeamlessM4T v2 large corre en Modal (GPU A10G).
Railway solo maneja: upload de video, ffmpeg, whisper (CPU, pequeño), y orquestación.

Variables de entorno requeridas en Railway:
  MODAL_SEAMLESS_URL  — URL del web_endpoint de Modal
                        Ej: https://<user>--video-dubber-seamless-api-web.modal.run
"""
...
# URL del worker Modal — obligatoria en producción
MODAL_URL = os.getenv("MODAL_SEAMLESS_URL", "").rstrip("/")
...
STEP_GROUPS = {
    ...
    "translation": {
        "label": "Traducción y Doblaje (GPU · Modal)",
        "steps": []
    }
}
...
class DubbingPipeline:
    ...
    async def run(self, job_id: str, jobs: Dict[str, Any]):
        ...
        try:
            if not MODAL_URL:
                raise RuntimeError(
                    "Variable MODAL_SEAMLESS_URL no configurada. "
                    "Despliega modal_worker.py con 'modal deploy modal_worker.py' "
                    "y añade la URL resultante como variable de entorno en Railway."
                )
    ...
    def _init_steps(self, job: Dict):
        ...
        for group_key, group in STEP_GROUPS.items():
            ...
            for step in group["steps"]:
                job["steps"][step["key"]] = {
                    "label": step["label"], "status": "pending",
                    "group": group_key, "started_at": None, "completed_at": None,
                }
    ...
    # ── FASE 1: IMPORTACIÓN (ffmpeg — Railway) ───────────────────────
    ...
    # ── FASE 2: TRANSCRIPCIÓN CON WHISPER (Railway CPU) ──────────────
    ...
    # ── FASE 3: DOBLAJE VÍA MODAL (GPU remota) ───────────────────────
    ...
    async def _run_translation_phase(self, job_id: str, job: Dict, target_langs: List[str]):
        ...
        # Leer el audio normalizado una sola vez y codificar en base64
        ...
        for lang in target_langs:
            ...
            call_key = f"modal_dub_{lang}"
            job["steps"][call_key] = {
                "label": f"Doblando en GPU · Modal ({lang_name})",
                "status": "pending", "group": "translation",
                "started_at": None, "completed_at": None,
            }

            await self._set_step(job, call_key, "running")

            # Llamada HTTP al worker Modal (timeout = 9 minutos)
            dubbed_b64 = await self._call_modal(audio_b64, seamless_lang)

            # Guardar WAV doblado localmente
            dubbed_wav = temp_base / f"dubbed_{lang}.wav"
            wav_bytes  = base64.b64decode(dubbed_b64)
            dubbed_wav.write_bytes(wav_bytes)
            ...
            await self._set_step(job, call_key, "complete")

        job["groups"]["translation"]["status"] = "complete"

    async def _call_modal(self, audio_b64: str, tgt_lang: str) -> str:
        """
        Llama al endpoint HTTP de Modal y devuelve el audio doblado en base64.
        Timeout de 9 minutos — suficiente para videos largos incluyendo cold start.
        """
        url = f"{MODAL_URL}/web_translate"
        payload = {"audio_b64": audio_b64, "tgt_lang": tgt_lang, "speaker_id": 0}
        ...
        if "error" in data:
            raise RuntimeError(f"Error en Modal worker: {data['error']}")
