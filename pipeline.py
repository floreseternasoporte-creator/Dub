...
FIXES aplicados:
  - OOM/Killed: carga con float16 + low_cpu_mem_usage + offload a disco si falta RAM
  - FutureWarning deepspeed: suprimido antes de cualquier import de transformers
  - FutureWarning resume_download: suprimido via env var antes de descarga
  - Limpieza de caché torch entre chunks para evitar acumulación de tensores
  - Manejo explícito de MemoryError con mensaje claro
  - Descarga directa a SEAMLESS_DIR (sin cache_dir doble-copia)
"""
...
# ── Suprimir FutureWarnings ANTES de importar transformers/huggingface ──
warnings.filterwarnings("ignore", category=FutureWarning, module="transformers.deepspeed")
warnings.filterwarnings("ignore", category=FutureWarning, module="huggingface_hub.file_download")
# Desactivar aviso de resume_download vía env (huggingface_hub lo respeta)
...
def _get_seamless():
    """
    Carga SeamlessM4T v2 large minimizando el uso de RAM:
      - float16 en lugar de float32  → ~50 % menos memoria
      - low_cpu_mem_usage=True       → carga shard a shard sin pico doble
      - device_map="auto"            → offload automático a disco/CPU si GPU no disponible
      - Se descarga directamente en SEAMLESS_DIR (sin cache_dir extra)
    """
    ...
    if _seamless_model is None:
        ...
        # Preferir float16; si la plataforma no lo soporta caemos a float32
            ...
            if (SEAMLESS_DIR / "config.json").exists():
                ...
                with warnings.catch_warnings():
                    ...
                    _seamless_model = SeamlessM4Tv2ForSpeechToSpeech.from_pretrained(
                        model_id,
                        torch_dtype=dtype,
                        low_cpu_mem_usage=True,  # ← clave anti-OOM
                        local_files_only=False,
                    )
                # Guardar local para carga rápida en reinicios
                print(f"[SeamlessM4T] Guardando en volumen {SEAMLESS_DIR} ...")
                _seamless_processor.save_pretrained(str(SEAMLESS_DIR))
                _seamless_model.save_pretrained(str(SEAMLESS_DIR))
                print(f"[SeamlessM4T] Guardado OK")

        except MemoryError as exc:
            raise RuntimeError(
                "Sin memoria suficiente para cargar SeamlessM4T v2 large. "
                "El modelo requiere al menos 8 GB de RAM libre. "
                "Aumenta la memoria del contenedor/instancia o usa un plan superior."
            ) from exc

        _seamless_model.eval()
        # Liberar memoria de construcción ya no necesaria
        gc.collect()
...
class DubbingPipeline:
    ...
    def _translate_audio_sync(self, audio_path: str, output_path: str, tgt_lang: str):
        """
        Speech-to-speech con SeamlessM4T v2 large.
        Procesa en chunks de 30s y libera tensores entre chunks para no acumular RAM.
        """
        import torch
        import torchaudio
        ...
        for idx, chunk in enumerate(chunks):
            ...
            # Pasar a numpy y liberar tensores inmediatamente
            output_chunks.append(out[0].cpu().numpy().squeeze())
            del out, inputs
            gc.collect()   # ← evita acumulación de RAM entre chunks
